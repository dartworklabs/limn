...
Every request is identified before it is dispatched (design: [access-and-sync.md](design/access-and-sync.md)):

1. **`Authorization: Bearer <token>`** — an agent API token from `limn token create <instance>` (or
   `limn token create --state-dir <dir>` for a plain `limn serve`). It is shown once; the server stores only its
   SHA-256 hash in `<state_dir>/tokens.json` (mode `0600`) and re-reads that file when it changes, so a
   `limn token revoke` takes effect on the next request. A valid token wins over any identity header and makes
   the requester the agent `{"login": "agent:<token name>", "name": "<token name>"}`. An unknown, revoked, empty
   or repeated Bearer header is `401` — it never falls back to another identity. Other `Authorization` schemes
   are ignored.
2. Otherwise the instance's identity provider (`--auth`, [operations.md](operations.md) §Command-line arguments):
   `tailscale` (default) reads `Tailscale-User-Login`/`-Name`/`-Profile-Pic` from a loopback TCP peer only;
   `local` makes every loopback request the owner; `trusted-proxy` reads the configured proxy headers from
   `--trusted-proxies` peers only.
3. Under `tailscale` only, a loopback request with neither is the agent `{"login":"local","name":"로컬/에이전트"}`
   — the v0.1 behaviour, **deprecated** (the server logs a warning; `--no-agent-loopback` makes it `401`).

Anything else is `401 {"error": …}` with `WWW-Authenticate: Bearer realm="limn"`.

```bash
export LIMN_TOKEN=limn_…            # from: limn token create <instance>
...
**Admission and roles.** People identified by a header are then checked against `--members-only` (only logins in
`people.json` or `--allow`) or `--allow` alone; a refused login gets `403` and is not recorded. Without either,
everyone the provider identifies is admitted and recorded in `people.json` on first visit without a `role` field.
Each person's optional `role` in `people.json` decides what they may change, enforced once before dispatch:

| Role | POST allowed |
| --- | --- |
| `viewer` | only `/api/pick` and `/api/revision-build` (computations); everything else `403` |
| `agent` (every token, the loopback agent, and people given this role) | everything except `/api/pins/{id}/confirm` (`403`); a close without `review` goes to pending review |
| `editor` (a person without a role) · `owner` | everything |

`GET` requests are open to every admitted role. Owner-only operations (members, tokens, settings) are CLI/file
level in v0.2 (`limn member`, `limn token`); there are no owner-only HTTP endpoints yet. `me` in `/api/meta` and
`/api/people`, and each entry of `/api/people`, carry an additive `role` field.
...
| `GET` | `/api/people` | @mention candidates: `{people:[{login,name,pic?,last_seen?,role}], me}` — §@mentions, people, and events, §Authentication (`role`). Read-only |
...
- An agent's `curl` has no identity headers, so `closed_by` is recorded as `local/agent` (`로컬/에이전트`); with a token it is `{"login":"agent:<token name>","name":"<token name>"}` (§Authentication).
...
- **People** (`<state_dir>/people.json`, `{"version":1,"people":[{login,name,pic?,first_seen,last_seen,role?}]}`): a person who opened this viewer (`GET /`, full `/api/meta`) or sent a write request, or was added with `limn member add`. The optional `role` (`owner`·`editor`·`viewer`·`agent`, absent = `editor`) is set with `limn member` and kept when the server rewrites the entry (§Authentication). Local/agent requests (including tokens) are never recorded. The same value is only rewritten once every 10 minutes (`/api/meta?light=1` polling never writes). `GET /api/people` merges this with pin authors, actors, and thread posters.
