...
| 역할 | 할 수 있는 일 |
| --- | --- |
| `owner` | 전부. 소유자 전용 HTTP 엔드포인트 `POST /api/clear`(모든 핀을 보관본으로 옮기고 비운다, 본문 `{"confirm": "clear all pins"}`, 0.2.1부터)와 휴지통의 영구 삭제 `POST /api/pins/{id}/purge`(0.2.2부터)도 포함한다. 나머지 소유자 작업(멤버·토큰·설정)은 CLI와 파일 수준이다 |
| `editor` | v0.1에서 사람이 하던 일 중 전체 지우기를 뺀 모두: 핀, 답글, 수정, 닫기, 확인, 다시 열기, 삭제·되살리기, 재빌드. 휴지통의 영구 삭제는 못 한다. **역할이 없는 사람은 editor다** |
...
| 저장소 금지 | 그 폴더가 git 작업 트리 안이고 저장소가 그 파일을 무시하지 않으면 `--save`가 멈춘다. `--force`로도 넘길 수 없다. 심링크는 풀어서 실제 폴더로 판단하고, 호출한 셸의 `GIT_*` 변수는 무시한다. git이 판단하지 못하면(소유권 경고 등) 역시 멈춘다 |
...
| 인스턴스 관리자 | 확인 요청에 이 파일의 토큰을 표준 입력으로 `curl`에 넘긴다(명령줄에 남지 않는다). 그 포트에서 듣는 프로세스가 모두 이 계정의 것일 때만 보낸다(Linux는 `/proc/net/tcp`의 uid, macOS는 `lsof`). 인스턴스가 꺼진 사이 다른 계정이 그 포트를 잡아 토큰을 받아 가지 못하게 하려는 것이다. 심링크, 다른 계정의 파일, 그룹·다른 사람 권한이 있는 파일, 토큰 한 줄이 아닌 파일은 쓰지 않고 경고한다 |
