...
- `doc` 은 그 핀이 속한 문서의 키다(§문서 매개변수). `doc` 이 없는 옛 핀은 첫 문서로 읽는다. 0.3.5 전에는 새 줄 핀이 보내는 `mention`·`assigned` 가 핀의 문서 대신 첫 문서의 키를 적었다. 그래서 여러 문서 인스턴스에서 둘째 문서의 새 핀 알림이 첫 문서를 가리켰다. 이미 쓰인 기록은 고치지 않는다.
