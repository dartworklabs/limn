...
지금 Limn은 **하나의 배포 단위(`dartwork-limn` 패키지) 안에 표준 라이브러리만 쓰는 큰 모듈 하나**가 대부분의 일을 한다. 아래는 2026-09-25 Limn 0.3.0 시점의 실측이다.

| 파일 | 줄 수 | 맡은 일 |
| --- | --- | --- |
| [`src/limn/server.py`](../../src/limn/server.py) | 10,680 | 설정, 빌드, 역변환, 핀 저장소, 검증, 사람·이벤트, 접근 제어, HTTP 처리, 그리고 뷰어 HTML·CSS·JS 전부 |
...
`server.py` 안은 `# ------` 배너 주석으로 관심사별 구역이 나뉘어 있다. 순서대로 문서(Doc)·빌드·빌드 이력·`--git-pull`·핀 단위 변경(0.3)·리비전 PDF·목차 라벨·보기 전용 PDF·meta·원문 접근·역변환 두 경로·범위 사다리·anchor·핀 저장소·위치 추정·겹침·입력 검증·핀 조작·사람과 이벤트·처리 중 표시·선택 해석·신원·접근 제어·뷰어·HTTP 처리기·입구다. 뷰어는 6,213행부터 9,826행까지 약 3,610행짜리 파이썬 문자열 `HTML` 하나에 들어 있다.
