...
또 `detect_main()`, `free_port()`, `clean_label()`은 문제가 있으면 `sys.exit()`로 프로세스를 끝낸다. 시작할 때만 불린다는 전제가 코드에 드러나지 않는다. (2026-09-26 바꿨다: 이 함수들과 `configure_access()`·`probe_port()`는 `StartupRefused(message)`를 돌려주고, `sys.exit`은 `main()` 한 곳에만 있다. `tests/test_server.py`의 `StartupRefusals`가 검사한다. 같은 날 뒤에 이 규칙들은 `limn/startup.py`로 옮겼고, `tests/test_startup.py`가 그 모듈에도 `sys.exit`이 없음을 검사한다.)
