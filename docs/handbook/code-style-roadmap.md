...
**바꾼 모습.** 2026-09-26 확인 전이를 처음으로 옮겼다. 규칙은 [`limn/pins/lifecycle.py`](../../src/limn/pins/lifecycle.py)의 순수 함수, 불러오기·저장은 `server.py`의 `confirm_pin()`(같은 날 뒤에 핀 서비스 [`limn/service/transitions.py`](../../src/limn/service/transitions.py)로 옮겼다), 응답은 HTTP 처리기가 맡는다. 동작과 응답은 그대로다.
...
**옮기기 전 모습.** `server.py`에서 `HTTPError`를 던지는 함수가 48개였다. `clean_note()` 같은 입력 검사 함수뿐 아니라 `confirm_pin()`, `reply_pin()`, `edit_pin()` 같은 핀 조작 함수도 직접 `HTTPError(409, ...)`를 던졌다.
...
또 `detect_main()`, `free_port()`, `clean_label()`은 문제가 있으면 `sys.exit()`로 프로세스를 끝냈다. 시작할 때만 불린다는 전제가 코드에 드러나지 않았다. (2026-09-26 바꿨다: 이 함수들과 `configure_access()`·`probe_port()`는 `StartupRefused(message)`를 돌려주고, `sys.exit`은 `main()` 한 곳에만 있다. `tests/test_server.py`의 `StartupRefusals`가 검사한다. 같은 날 뒤에 이 규칙들은 `limn/startup.py`로 옮겼고, `tests/test_startup.py`가 그 모듈에도 `sys.exit`이 없음을 검사한다.)

이렇게 되면 같은 규칙을 HTTP가 아닌 입구(예: 앞으로의 CLI 명령이나 백그라운드 작업)에서 쓰기 어렵다. 한국어 오류 문구도 48곳에 흩어져 있었다.

**바꾼 모습.** 요청 파싱은 HTTP 층(`web/`)이 맡는다. 파서는 검증된 값이나 이유가 붙은 거절 값을 돌려준다. 위의 `clean_note()`는 지금 [`limn/web/parse.py`](../../src/limn/web/parse.py)의 이 함수다.
...
본문 파서(`parse_add` 등)는 필드마다 이 값을 확인해 첫 거절을 그대로 돌려주고, 처리기는 `accepted()`로 옛 400 문장 그대로 답한 뒤 서비스에 파싱된 값(`AddRequest`)만 넘긴다. 도메인은 `PinStillOpen`·`AgentCannotConfirm`처럼 이유를 담은 값을 **돌려준다**(`confirm_pin(...) -> DonePin | AlreadyDone | PinStillOpen | AgentCannotConfirm | PinNotFound`). HTTP 층(`web/answers.py`)은 `match`로 받아 이유별 상태 코드와 한국어 문구를 고른다. `sys.exit()`는 `main()` 한 곳에서만 부르고, 안쪽 함수는 시작 실패 이유를 값으로 돌려준다. 2026-09-26 현재 `server.py`에 `raise HTTPError`는 없다. `HTTPError`를 던지는 곳은 HTTP 층(`web/handler.py`·`web/answers.py`)과, 일부러 예외로 둔 신원·입장·역할 판단(`limn/access.py`, R10)뿐이다.
...
**지금 모습 (2026-09-26).** `cur_doc()`은 없다. `C.`는 `server.py` 한 파일에만 남았고, 코드에서 100곳(주석까지 세면 107곳)이다. 모두 시작 단계가 `C`를 채우는 줄이거나, 요청마다 설정 값을 만들어 옮긴 모듈에 넘기는 연결(`build_config`·`pin_store`·`meta_settings`·`access_settings`·`pick_context` 등)이다. 옮긴 모듈은 `C`를 읽지 않는다(모듈마다 import·참조 검사).
...
**지금 코드의 모습.** `server.py` 함수 353개 중 136개에 docstring이 없었다(0.2.2). 모듈을 옮기면서 옮긴 함수에는 모두 달았고, 2026-09-26 조립 지점만 남은 `server.py`는 함수 124개 중 6개(`now_str`·`multi_doc`·`who`와 레코드 검사 도우미 셋)에만 없다. `migrate.py`(10개 중 9개)와 `cli.py`(22개 중 5개, 클래스 `CliError`도)에도 없었는데, 2026-09-26 두 파일을 타입 검사에 넣으면서 모두 달았다 (R8). 있는 것 중에는 `pin_state()`처럼 이유까지 설명하는 훌륭한 글이 많다. 반대로 없는 곳에서는 중요한 계약이 숨는다.
...
또 주석이 `§P0c-C`, `§P0b-보완 C`, `P0b-01` 같은 옛 작업 계획 번호를 가리켰다. 그 계획 문서는 이 저장소에 없다. 2026-09-26 `src/`의 이런 참조 23곳(파이썬 12곳, 뷰어 JS 11곳)을 Handbook의 절 이름으로 바꿨고, `tests/test_server.py`의 것은 같은 날 그 파일을 나누면서 바꿨다 (예: `docs/handbook/api.md §처리 중 표시 (claim)`). 새 주석은 처음부터 Handbook 절을 가리킨다.
...
- `server.py` (2026-09-26, 137건): 조립 지점만 남은 뒤 마지막으로 목록에 넣어, 패키지의 모든 모듈이 strict 검사 대상이 됐다. 대부분 타입 인자 없는 `dict`·`list`·`tuple`과 표기 없는 인자·반환, `None` 기본값에 `None`이 빠진 타입(11곳)이었다. 표기에는 옮긴 모듈의 타입을 그대로 썼다: `Doc`, 저장된 레코드 `Row`(쓰는 쪽)·`Record`(읽기만 하는 쪽), `Json`·`Event`, `BuildResult`, 원고 이력의 거절 묶음(`DiffRefusal` 등), 핀 조작의 결과 합 타입, `CloseChange`, `access.Principal`, 시작 단계의 `argparse.Namespace`. `_is_int`·`_is_num`·`_is_str_list`는 `TypeGuard`로 적어 레코드 검사의 비교가 좁혀진다. 새 `TypedDict`는 만들지 않았다 — 레코드와 응답은 옮긴 모듈이 이미 `Mapping[str, Any]`·`dict[str, Any]`로 다루고, 모양의 정본은 [api.md](api.md) §핀 레코드 스키마와 계약 스냅숏 테스트다. 처리기는 `_ModuleApp`(모든 속성이 `Any`)으로 `server.py`에 닿으므로, 그대로면 빠지거나 서명이 틀린 연결을 mypy가 보지 못한다. 그래서 `server.py` 끝의 `if TYPE_CHECKING:`에서 모듈 자체를 `limn.web.app.App`에 대입해 검사한다(연결 하나의 이름을 바꾸거나 인자 타입을 틀리게 하면 실패함을 확인). 이를 위해 `App`의 실행 설정·문서·신원은 좁은 프로토콜 대신 `server.py`의 실제 타입(`Cfg`·`Doc`·`access.Principal`)을 가리키고, 다른 모듈에서 가져온 `App` 멤버는 `X as X`로 다시 내보낸다. 검사가 드러낸 어긋남 넷은 표기만 넓히거나 좁혀 맞췄다: 찾기 함수 `Locator`는 읽기만 하는 레코드(`Mapping`)를 받고, `locate_file`은 빌드 폴더만 읽으므로 원고 이력의 문서(`RevisionDoc`)도 받으며(`BuildRoot`), `PinContext`의 `note_tags`는 힌트를 두 번 읽으므로 `Sequence`를 받고(부르는 쪽은 모두 튜플을 넘긴다), `access.file_present`는 `TypeGuard[Path]`다. 동작은 바뀌지 않았다(전체 테스트 녹색).
...
**지금 코드의 모습.** Limn은 이미 이 부분을 조심스럽게 다룬다. 요청 본문을 끝까지 읽어 요청 밀수(smuggling)를 막고, Host·Origin을 검사하고, 신원 헤더 없는 테일넷 요청을 거절한다. 0.2.0부터는 신원 방식·토큰·역할이 더해져 신뢰 경계가 넓어졌다. 토큰은 해시로만 저장하고, 역할은 처리기 한 곳에서 집행하며, `tests/test_access.py`가 거부 경우를 확인한다 ([ADR-0002](../adr/0002-access-control.md)). 2026-09-26부터 이 경계의 코드는 [`limn/access.py`](../../src/limn/access.py) 한 모듈에 있다. 이 모듈은 실행 설정 `C`를 읽지 않고 서버를 가져오지 않는다. 설정은 `AccessSettings`, 요청 때 읽는 파일 사실(토큰 목록·역할)은 `AccessLookups`로 받으므로 권한 규칙을 서버 없이 테스트할 수 있다. `tests/test_access_module.py`가 모듈 수준에서 거부 경우를 확인한다(잘못된·폐기된 토큰, 원격 피어의 신원 헤더, 신뢰하지 않는 프록시, 멤버 아님, 역할별 POST 등). 이 경계의 거절은 코딩 규칙의 예외로 값이 아니라 `HTTPError`를 던진다(fail closed): 던진 거절은 부르는 쪽이 놓칠 수 없지만, 값으로 돌려준 거절은 새 호출자가 무시하면 요청이 통과한다.

**바꾼 모습.** 새로 바꿀 것은 없다. 경계를 건드리는 PR은 PR 설명에 "보안 경계 변경"을 적고, 부정 경우 테스트를 함께 올린다. ADR-0002의 v0.3 이후 단계는 spec을 먼저 승인받고 시작한다. R1을 적용할 때 역할 판단("이 역할이 이 동작을 해도 되는가")도 순수 판단으로 꺼내면 권한 규칙을 파일 없이 테스트할 수 있다. (2026-09-26 `limn/access.py`로 옮기면서 설정과 파일 사실을 인자로 받게 했다. 거절을 던지는 방식은 위 이유로 그대로 둔다.)
...
| 6 HTTP와 조립 지점 | 완료 | 2026-09-26 소유자 결정으로 4단계의 `store.py`와 나란히 시작. 앞부분: HTTP 처리기(`Handler`·`Server`·`Server6`, 본문 읽기와 1 MiB 한도, Host/Origin 검사 호출, GET/POST 경로 분기), 결과별 응답(옛 `_*_answer`·`_*_reply` 메서드 → `limn/web/answers.py`의 함수), `HTTPError`·`InputRejected`·`SCOPE_REJECTIONS`·거부된 첫 화면을 `limn/web/`로 옮겼다. 처리기는 `server.py`를 가져오지 않는다. `server.py`가 자기 전역을 요청 때 그대로 읽는 보기(`_ModuleApp`)로 하위 클래스 `Handler`를 묶고, 처리기가 부르는 서비스 목록은 `web/app.py`의 `App` 프로토콜이다. 신원·역할·Host/Origin 판단(`identify`·`admit`·`check_role`·`host_ok`·`origin_ok`)은 보안 경계라 `server.py`에 그대로 두었다. 경로 표는 만들지 않았다 — 분기 순서가 응답을 정하므로(`/pages/`가 404로 떨어지는 길, `/api/pins/dropped`가 `/api/pins/N`보다 먼저) 이번에는 옮기기만 했다. 옛 코드와 응답(상태, Date·Server를 뺀 헤더, 본문)과 상태 디렉터리 전체 바이트가 같음을 160개 경우의 차등 비교로 확인(모든 핀 조작과 거절, 잘못된 JSON·본문 한도·`Transfer-Encoding`, 잘못된 Host·Origin, 테일넷의 헤더 없는 요청, 토큰 에이전트, 역할별 403, 거부된 첫 화면 ko·en, 500, 핀 단위 변경 거절 표). 뒷부분 첫째(요청 파서): 본문·쿼리의 파서를 모두 `limn/web/parse.py`로 옮겼다. 파서마다 값이나 `InputRejected`를 돌려주고(옛 `clean_*`는 `HTTPError`를 던졌다), 처리기가 `accepted()`로 옛 400 문장 그대로 답한 뒤 서비스에 파싱된 값을 넘긴다(`add_pin(D, AddRequest, …)`, `edit_pin(pid, EditRequest, …)`, `pick(D, PickRequest)` 등). 필드를 보는 순서와 부수효과와의 순서(예: 잘못된 `?ev=`는 meta를 만든 뒤에 거절)는 옛 코드 그대로다. 원고와 맞춰 보는 파서(새 핀·편집 위치·선택·snippet)는 파일의 줄과 빌드의 쪽을 `DocumentFacts`로 받고, 조립 지점이 `document_facts(D)`로 문서마다 만든다. 원고 트리 경로 규칙은 `limn/files.py`의 `file_in_tree` 하나로 모아 파서와 선택 해석이 같이 쓴다. `server.py`는 더 이상 `InputRejected`를 만들거나 가져오지 않는다(`raise HTTPError` 82 → 47곳, `cur_doc()` 35 → 28곳). 옛 코드와 응답·상태 디렉터리 전체 바이트가 같음을 314개 요청(파서의 모든 거절, 여러 문서의 보기 전용 PDF 핀 포함)의 차등 비교로 확인. 뒷부분 둘째(결과 값): 원고 이력·핀 단위 변경·비교 PDF를 `limn/revisions.py`(git 쪽)와 `limn/scope.py`(순수 판단)로 옮기며 거절을 값으로 바꿨다. 경로의 거절은 경우마다 한 타입(`NoHistory`·`CommitNotRecent`·`DiffFailed`·`DiffUnavailable`·`NotInRepo`·`NoParent`·`UnsafeCache`·`AllSlotsBusy`·`DocumentBusy`·`RevisionNotReady`·`RevisionPdfMissing`, 경로마다 `DiffRefusal` 등으로 묶음)이고 `answers.revision_answer`가 옛 상태·문구로 답한다. `ScopeRejected` 예외는 경우별 값 `ScopeRefusal`이 됐고 `SCOPE_REJECTIONS`는 그 타입을 열쇠로 하나로 남았다. 비교 빌드의 단계 실패는 `StepFailed(kind)` 하나(모든 경우가 같은 데이터·같은 처리: 상태 파일에 기록)이고 문구 표 `REVISION_FAILURES`는 `web/errors.py`에 있다. 워커는 조립 지점이 넘긴 `revision_failure_text`로 옛 문구를 그대로 기록한다. 커밋 형식 검사(`bad_commit`)는 요청 파서로 옮겼다. 문서 조회는 `DocNotFound`를 돌려준다. `raise HTTPError`는 `server.py`에서 47 → 16곳(남은 것은 신원·입장·역할 판단). 옛 코드와 응답·상태 디렉터리 전체 바이트가 같음을 git 저장소를 둔 346개 요청의 차등 비교로 확인(핀 단위 diff, 비교 빌드 실패 기록, 슬롯·문서 잠금 409, 캐시 심링크 503, 첫 커밋 422 포함). 뒷부분 셋째(문서는 인자, R5): 스레드 지역 "현재 문서"(`cur_doc()`·`using_doc()`)를 없앴다. 처리기가 요청의 문서를 찾아 모든 서비스에 넘기고(`meta(D, …)`·`build_all(D)`·`build_state_snapshot(D)` 등), 빌드 스레드는 자기 문서로 시작한다. 빌드의 옛 이름 셸 22개(`cur_pages()`·`src_mtime()`·`seed_builds()`·`_read_head()` 등)를 지우고 부르는 쪽이 `limn.build`에 문서와 설정을 넘긴다. 남은 빌드 연결은 `build_all(D)`·`build_async(D)`·`_build(D)`·`_render_pdf_doc(D)`로, `BuildConfig`·`--git-pull`·보기 전용 그리기를 묶는 조립 지점의 일이다. `Doc`은 `limn/documents.py`로 옮겨 실행 경로를 만들 때 받은 `RunPaths`(조립 지점의 `C`)에서 읽는다. `cur_doc()`은 `server.py`에서 28 → 0곳, 처리기의 `app.cur_doc()`도 0곳. `C.`는 219 → 210곳. 옛 코드와 응답·상태 디렉터리 전체 바이트가 같음을 355개 요청(여러 문서의 실제 비동기 빌드 포함)의 차등 비교로 확인. 뒷부분 넷째(조립 지점): `main()`은 인자를 파싱해 `start()`에 넘기고, 거절이면 그 문구로 끝내고 아니면 서버를 돌린다. `start()`는 단계를 차례로 부른다: 접근 설정(`configure_access`), `--port` 확인(`probe_port`), 실행 설정과 문서(`configure_run`), 저장소와 빌드와 감시 스레드(`prepare`), 시작 요약(`report`), 듣는 서버(`listen`). 시작을 막는 단계는 모두 `StartupRefused`를 돌려주고 `sys.exit`은 `main()`에만 있다(R3). 순서와 문구, 상태 폴더를 만드는 시점은 그대로다. 옛 코드와 거절 20가지·정상 시작 3가지의 종료 코드·stdout·stderr·상태 폴더가 같음을 따로 돌린 시작 차등 비교로 확인. 신원·입장·역할 판단(`identify`·`admit`·`check_role`·`bearer_of`)은 옮기지 않았다. 옮기는 것이 기계적이지 않다: 이 함수들은 실행 설정 13개와 파일에 기댄 조회 4가지(`tokens.json`·`people.json`과 그 캐시, 에이전트 토큰 파일 안내)를 읽는데, 그 조회를 CLI와 `pins.md`도 쓴다. `web/`으로 옮기려면 이것들을 모두 인자로 넘겨야 해서 경계를 옮기는 게 아니라 다시 쓰는 일이 된다(같은 날 뒤에 그렇게 했다 — 아래 다섯째). 또 이 경계의 거절은 일부러 예외(`HTTPError`)다. 던진 거절은 부르는 쪽이 놓칠 수 없지만, 값으로 돌려준 거절은 새로 짠 호출자가 무시하면 요청이 통과한다. 그래서 `server.py`가 `limn.web`에서 가져오는 것은 이 네 함수의 `HTTPError`·`CONFIRM_BY_HUMAN`(과 저장소에 넘기는 거절 타입)으로 좁혀졌다. `C.`는 `server.py`에서 210 → 208곳이고, 그중 124곳이 조립 지점에 있다: 시작 단계(`configure_access`·`access_log_lines`·`configure_run`·`prepare`·`report`·`listen`·`start`와 `init_doc`·`tighten_state_perms`) 114곳, 요청마다 서비스에 넘기는 연결(`build_config`·`pin_store`·`document_facts`·`revision_context`) 10곳. 나머지 84곳은 서비스 안에 있다. 이어서 `pins.md` 렌더를 `limn/pins/render.py`로 옮겼다: `pins_md_text(PinsMdInput)`은 순수 함수이고, 렌더가 읽던 것(실행 설정, 문서와 빌드 도장, 시계, 토큰 파일이 있는지와 홈 폴더, `people.json`, 핀마다의 파일 위치와 긴 줄 길이, 겹침 배지, @태그, 스레드 회차)은 조립 지점의 `pins_md_input`이 값으로 모아 넘긴다. 저장소에는 전처럼 `server.pins_md_text`가 렌더로 넘어간다. 토큰 파일 문구(`UNAUTHENTICATED`·`shell_path`·`token_file_curl`·`loopback_refused_text`)는 `pins.md`와 접근 검사가 같이 쓰므로 순수 모듈 `limn/guidance.py`에 하나로 두었다. `C.`는 `server.py`에서 208 → 205곳. 옛 코드와 `pins.md`·`GET /pins.md`·`GET /api/pins`·401 본문·상태 디렉터리 전체 바이트가 같음을 276개 관찰(한 문서·세 문서·설정에 없는 문서, 줄·영역 핀, 열림·처리 중·검토 대기·완료·확인, 다시 연 핀과 긴 스레드, 질문·담당·참고 태그, 옛 레코드, 긴 줄 인용, 토큰 파일 있음·없음·공백 경로·홈 밖, 루프백 에이전트 끔, 테일넷 에이전트)의 차등 비교로 확인. 이어서 meta와 문서 조회: 뷰어가 폴링하는 읽기(`/api/meta`·`/api/docs`·`pins_rev`·목차 라벨)를 `limn/meta.py`로, `.aux` 목차 줄의 파서를 순수 모듈 `limn/outline.py`로, 문서 목록을 인자로 받는 조회(`request_doc`·`doc_for_file`·`pin_doc_key`)와 `DocumentFacts`·`to_source`를 `limn/documents.py`로, 쪽 목록(`page_list`)과 보기 전용 PDF 그리기(`render_pdf_doc`·`pdf_changed`)를 `limn/build.py`로, `tex_lines`·`vendor_file`을 `limn/files.py`로 옮겼다. 설정은 `MetaSettings`로, `--git-pull` 상태와 시계는 값으로 받는다. 핀 개수는 동기화 쓰기가 있는 `snapshot_pins()`를 거치므로 `server.meta()`가 라이트가 아닐 때만 덧붙인다. 문서 목록 `DOCS`는 조립 지점에 남고 `server.py`는 목록과 설정을 넘기는 한 줄짜리 연결만 둔다. 옛 코드와 응답(상태, Date·Server를 뺀 헤더, 본문)·상태 디렉터리 전체 바이트가 같음을 따로 돌린 177개 경우의 차등 비교로 확인(meta 전체·라이트·`ev`, 한 문서·여러 문서·보기 전용 PDF, 목차 라벨의 `.aux` 없음·심볼릭 링크·크기 초과·잘못된 UTF-8·200행 한도, `/pdf`·`/pages`·`/vendor/pdfjs`의 거절, 파일로 문서를 고르는 새 핀, 쪽 수를 넘는 영역 핀과 선택, pdftoppm으로 실제 그린 보기 전용 PDF의 다시 그리기·실패·파일 없음). 이어서 사람·@태그·알림·감사 기록을 옮겼다: `people.json`은 `limn/people.py`(항목 검사·저장 형식·기록, @태그 후보), @태그의 순수 규칙은 `limn/mentions.py`(풀기, 지금 차례, `addressed`·`fyi`, 메모 태그와 재알림 간격), 알림은 `limn/events.py`(이벤트 한 건과 받는 사람·폴링 고르기는 순수, `events.jsonl`은 `EventLog`), 감사 기록은 `limn/audit.py`, 프로세스 간 잠금 `store_lock`은 `limn/files.py`. 새 모듈은 서버를 가져오지 않고 `C`를 읽지 않는다(모듈마다 import 검사). 파일 위치·잠금·읽기 캐시·기록 메모·시계·에이전트 판단·문서 키 조회는 조립 지점이 `people_book()`·`event_log()`와 옛 이름 셸(`record_person`·`known_people`·`make_event`·`emit_events`·`note_tags`·`events_since`)로 넘긴다. 핀 조작 셸이 이벤트를 모아 핀 쓰기 뒤 `PIN_LOCK` 아래에서 내보내는 순서는 그대로다. `C.`는 `server.py`에서 203 → 200곳. 옛 코드와 응답·상태 디렉터리 전체 바이트(`events.jsonl`·`audit.jsonl`·`people.json`·토큰·핀 파일과 권한)가 같음을 123단계 차등 비교로 확인(모든 알림 종류, 겹치는 첫 이름과 힌트·자기 태그·에이전트 역할 태그·메일 주소, 메모 재알림 간격, `?ev=` 폴링, 회전, clear·purge·토큰·멤버 감사, 심볼릭 링크 거부). 이어서 핀 위치 서비스를 옮겼다(R1·R5): 순수 규칙은 `limn/pins/position.py`(위치 추정 `pin_est`와 그 재료 `EstContext`, 겹침 `overlaps_by_id`·`selection_rel`·`overlaps_for_range`, anchor 재동기화 `follow_anchor`·`resync` — 핀 하나의 바뀐 레코드 사본을 돌려준다), 부수효과는 `limn/locate.py`(SyncTeX·pdftotext, `.tex` 읽기, 토큰 가중치 캐시 `TokenCache`, 핀 파일 찾기 `locate_file`·`pin_location`, `sync_all`, 빌드 이력 읽기 `est_context`, 선택 해석 `pick`과 snippet)다. `locate.py`는 문서와 설정(`PickContext`)을 인자로 받고 `C`·서버·HTTP 층을 모른다(`tests/test_locate.py`). 문서 목록과 실행 설정을 묶는 연결(`pin_location`·`stamp_location`·`pin_locator`·`sync_all`·`overlaps_by_id`·`overlaps_for_range`·`pick_context`·`pick`·`snippet_api`·`overlaps_api`)만 `server.py`에 남고, 저장소는 전처럼 `pin_store()`로 `sync_all`을 받는다. `server.py` 3,525 → 3,108행, `C.` 200 → 198곳(서비스 안의 `C.`가 연결로 옮겼다; 앞의 이동들을 합친 뒤의 수). 원고 줄 읽기 `tex_lines`와 SyncTeX 경로 되돌리기 `to_source`는 먼저 옮긴 `limn/files.py`·`limn/documents.py`의 것을 `locate.py`가 가져다 쓴다. 옛 코드와 응답·상태 파일 바이트가 같음을 따로 돈 두 프로세스의 차등 비교 398개 경우로 확인(가짜 `synctex`·`pdftotext`로 줄 핀의 pick 11가지와 거절 6가지, 보기 전용 pick 3가지, 같은·다른·해시 없는 원고의 재빌드 뒤 `est`, 빌드 없는 옛 핀, 원고 편집 뒤 anchor 이동·잃음, anchor 없는 옛 핀, 옮긴 체크아웃의 레코드, 휴지통 되살리기와 편집의 위치 기록, `rel`·`/api/overlaps`·`/api/snippet`·`pins.md`·meta). 뒷부분 다섯째(접근 제어, R10): 신원·입장·역할·Host/Origin 판단과 토큰·멤버 상태 도우미를 `limn/access.py`로 옮겼다. 이 모듈은 `C`를 읽지 않고 서버를 가져오지 않는다. 조립 지점이 요청마다 실행 설정 11개를 `AccessSettings`로, 파일에 기댄 조회(`tokens.json` 항목, `people.json` 역할, 한 번만 내는 로컬 에이전트 경고)를 `AccessLookups`로 넘긴다. Host/Origin 판단은 `--public-host` 목록을, `pins.md` 기준 주소는 거기에 포트를 더 받는다. 두 캐시(`FileCache`: 경로와 inode·mtime·크기가 열쇠, 바뀔 때만 다시 읽음)와 경고(`WarnOnce`)의 주인은 `server.py` 하나다. `limn token`·`limn member`는 `limn.access`를 직접 부르고, 감사 기록(`cli_audit`)과 `people.json` 형식(`PEOPLE_FORMAT`)만 `server.py`에서 받는다(`PEOPLE_FORMAT`은 `limn.people`의 검사와 저장 형식을 묶어 넘긴다. `access.py`는 사람 저장소를 가져오지 않는다). 토큰 파일 안내 문구(순수 모듈 `limn/guidance.py`)와 프로세스 사이 잠금(`limn/files.py`의 `store_lock`)은 앞서 옮긴 것을 같이 쓴다. 거절은 합의대로 계속 `HTTPError`를 던지고 상태·`reason`·문장·안내 페이지는 그대로다. `raise HTTPError`는 `server.py`에서 16 → 0곳, `C.`는 198 → 194곳, `server.py` 3,108 → 2,576행(앞의 이동들을 합친 뒤의 수). 옛 코드와 따로 돌린 프로세스에서 186개 경우가 같음을 확인했다: 요청 122개(루프백 에이전트 켬·끔과 토큰 파일 있음·없음, 신원 헤더 있는·없는 테일넷 요청의 JSON과 한국어·영어 안내 페이지, trusted-proxy의 맞는·틀린 프록시 IP와 헤더, 위조 Host·Origin, 올바른·틀린·빈·두 번 온·폐기된 토큰, viewer·editor·owner·agent 역할, members-only, `--allow`, `--auth local`, `--public-host`), `limn token`·`limn member` 명령 42개(stdout·stderr·종료 코드), 시작 설정 18가지(거절 문구와 접근 요약 줄), 상태 폴더 네 번의 전체 바이트와 권한(`tokens.json`·`people.json`·`audit.jsonl` 포함). 남은 일: 서비스(휴지통)가 `C`를 읽지 않고 설정을 인자로 받게 하기. 그러면 `server.py`에는 조립 코드만 남는다. 이어서 그 일로 핀 서비스 셸 전부를 `limn/service/`(`add_edit.py`·`transitions.py`·`claim.py`·`trash.py`, 협력자 값 `PinContext`는 `context.py`)로 옮겼다. 서비스는 `C`·`PIN_LOCK`·`now_str`을 읽지 않고, 저장소·시계(`now_str`·`time.time`·덧붙인 메모의 시각)·알림 만들기와 내기·감사 기록·@태그 조회·파일 위치·`THREAD_MAX`·`TRASH_DAYS`·휴지통 점검 시각을 받는다. 처리기가 부르는 `App`의 이름·인자는 그대로라 `server.py`에는 한 줄 연결이 남고 `web/app.py`는 바꾸지 않았다. `server.py` 2,576 → 2,192행, `C.` 194 → 186곳, `PIN_LOCK` 22 → 9곳. 옛 코드와 따로 돌린 프로세스에서 102단계가 같음을 확인했다(응답 상태·본문과 단계마다 상태 폴더 전체 바이트: `pins.jsonl`·`pins.md`·`pins.dropped.jsonl`과 그 손상 백업·`pins.seq`·`events.jsonl`·`audit.jsonl`·`people.json`·clear 보관, 그리고 stderr의 로그 줄). 모든 거절을 지났다: 에이전트의 확인, 이미 완료, 아직 열림, 다른 사람의 claim, 닫힌 핀의 claim, claim 없음, 휴지통에 없음, 이미 살아 있음, 편집 거절 다섯(낡은 `base_rev`, 닫힌 핀의 위치 변경, 메모 길이, 파일 밖 줄, 원고 밖 핀), 스레드 가득 참, 답글의 다시 열기(사람·에이전트·질문 핀·사람 태그·에이전트 역할 태그), 없는 핀. 이어서 핀 목록의 계산 필드와 겹침 연결을 옮겼다: `GET /api/pins`·`GET /api/pins/dropped` 본문은 순수 모듈 `limn/pins/view.py`(`pins_payload`·`dropped_payload`, 레코드를 보이는 `public`·핀의 문서·문서마다의 빌드 이력·겹침·시계는 인자)이고, `state`를 내는 `pin_state`는 상태 타입의 이름이라 `parse_pin`과 규칙이 하나(`limn/pins/model.py`의 `state_of`)가 됐다. 겹침을 지금 찾은 파일로 세는 일과 `GET /api/overlaps` 본문은 `limn/locate.py`(`overlaps_by_id`·`overlaps_for_range`가 찾기 함수를 인자로 받는다, `overlaps_api`), `pins.md` 겹침 배지(`rel_badge`·`josa`)는 `limn/pins/render.py`로 갔다. 같은 규칙 두 벌도 하나로 모았다: 이벤트 `excerpt`는 `pins.md`의 한 줄 규칙 `render.flat`을 쓰고(순수 모듈이 주인이고 부수효과 모듈 `events.py`가 가져온다), `access.py`는 `people.json` 형식을 `server.py`의 `PEOPLE_FORMAT`으로 넘겨받지 않고 `limn/people.py`에서 직접 가져온다(`people.py`는 표준 라이브러리와 `limn/files.py`만 가져와 순환도 보안 경계 밖 의존도 없다, import 검사). 요청 문서·파싱 사실 구역(`request_doc`·`document_facts`·`assignee_people`)과 선택 해석 구역(`pick_context`·`pick`·`snippet_api`)은 이미 `App` 멤버를 이 인스턴스의 설정에 묶는 한 줄짜리 연결이라 조립 지점에 남겼다. `server.py` 2,192 → 2,109행, `C.` 186곳 그대로(핀 서비스 이동을 합친 뒤의 수). 옛 코드와 따로 돌린 프로세스에서 109개 경우가 같음을 확인했다(모든 상태·옛 claim·겹침·추정·옮긴 체크아웃·설정에 없는 문서·보기 전용 핀이 섞인 `GET /api/pins`·`?all=1`·핀 하나, 휴지통의 순서·`expires_ts`·만료·시각 없는 항목, `pins.md`, meta, `/api/overlaps`·`/api/snippet`의 정상과 거절, 가짜 SyncTeX·pdftotext로 pick 12가지, 답글·닫기·버림 뒤의 `events.jsonl`·`pins.md`·상태 파일 바이트, `limn member`·`limn token` 명령 14개와 `people.json`·감사 기록. `pins_rev`의 실제 mtime만 가렸다). 뒷부분 여섯째(`--git-pull`과 원격 main 감시): pull과 감시를 `limn/gitsync.py`(git 호출 순서, 여러 문서의 pull 나눠 쓰기 `PullShare`, 감시 한 바퀴·상태·루프 `SyncWatch`)와 순수 규칙 `limn/pull.py`(pull 결과 값 `Pulled`·`UpToDate`·`PullSkipped`·`PullFailed`, git 답 읽기, 감시 상태, 다시 빌드할 문서, `updating`이 끝나는 조건)로 옮겼다. 옛 `git_pull_phase`의 사전은 결과 값이 되었고, 빌드의 `pull`과 meta의 `sync`에 싣는 사전은 `pull_record()`가 옛 키 순서 그대로 만든다. 원고 폴더·문서 목록·`--git-pull`·git 실행기·시계·빌드 시작 함수는 조립 지점이 호출마다 넘긴다. 감시 스레드는 여전히 `prepare()`가 시작하고 본문(`SyncWatch.watch`)만 옮겼다. 이어서 뷰어 조립(`load_viewer_html`·`viewer_manifest`, 자리 표시자를 채우는 `replace()` 줄, Lucide 표 `LUCIDE`·`icon_svg`, `PDFJS_VERSION`, 메시지 표 읽기 `load_ui_messages`)을 `limn/viewer/assemble.py`로 옮기고 한 함수 `viewer_html(directory, messages, pdfjs_version=, mark=, icons=)`로 모았다. `viewer/`는 패키지가 됐다. `server.py`에는 `UI_EN`(처리기의 거부된 첫 화면도 읽는다)·`VENDOR_MIME`(PDF.js 경로)·`HTML = viewer_html(...)`·`build_html`이 남는다. `server.py` 1,951 → 1,810행. 옛 코드와 따로 돌린 프로세스에서 조립한 `HTML` 전체 문자열, 메시지 표, 아이콘마다의 `<svg>`, 라벨 셋의 `build_html()`, `GET /`·`/vendor/pdfjs/`의 응답 바이트(200·403·404와 거부된 첫 화면의 한국어·영어), 상태 폴더 바이트까지 35개가 같고, 만든 휠을 푼 트리로 돌려도 같았다. 보기 전용 PDF 다시 그리기 판단(`refresh_pdf_doc`)은 빌드 시작 함수를 받아 `limn/build.py`로 갔다. 원고 이력 구역은 이미 `limn/revisions.py`에 있고, 남은 것은 프로세스의 범위 캐시·작업 목록과 `revision_context()` 연결이라 조립 지점에 그대로 두었다. `server.py` 2,109 → 1,951행(앞의 이동들을 합친 뒤의 수). 옛 코드와 따로 돌린 프로세스에서 115개 경우가 같음을 확인했다(실제 임시 저장소, 고정한 시계·커밋 시각·경로): 재빌드의 pull 결과 전부(`up_to_date`·`ok`·`dirty`·`diverged`·`no_upstream`·`fetch_failed`·`not_git`, 여러 문서의 `shared`와 창이 지난 뒤), 감시 한 바퀴와 meta의 `sync`(`checking`·`updating`·`current`·`build_failed`·`deferred`와 3초 재시도·`not_main` 두 가지·`dirty`·`fetch_failed`·`unexpected`·`disabled`, 한 문서와 세 문서), 원고 이력·핀 단위 변경·비교 PDF 경로(TeX 격리 도구가 없어 비교 빌드는 실패 기록까지), 보기 전용 PDF 다시 그리기(바뀜·바쁨·LaTeX 문서·실제 그리기 뒤), 상태 폴더 전체 바이트. 벽시계에서 오는 값 둘(옮기지 않은 빌드가 짓는 쪽 디렉터리 이름, 빌드 안의 `git merge`가 고친 파일의 `src_mtime`)만 맞춰 비교했다. 뒷부분 일곱째(시작 규칙): 입구 구역의 규칙을 `limn/startup.py`(접근 설정 판단 `access_options` → `AccessOptions` 또는 `StartupRefused`, 시작 로그 `access_log_lines`, 포트 `free_port`·`probe_port`·`listen_refusal`, `--doc` 해석과 메인 파일·상태 폴더 `parse_doc_arg`·`make_docs`·`detect_main`·`pick_documents`·`state_dir`, `people.json` 권한 조이기, 라벨·색 `run_label`·`run_accent`, 시작 요약 줄 `summary_lines`), 명령줄 파서를 `limn/args.py`(`serve_parser`: 서버의 설명·버전·`--float-envs` 기본값은 인자), 실행 설정의 타입 `Cfg`를 `limn/config.py`로 옮겼다. 세 모듈은 `C`를 읽지 않고 서버를 가져오지 않으며 strict 타입 검사 목록에 들었다. `C`를 고치던 함수는 값을 돌려주는 규칙과 그 값을 `C`에 적용하는 `server.py`의 줄로 나눴다: `configure_access`는 `AccessOptions`의 필드를 같은 이름으로 옮기고(거절이면 아무것도 적용하지 않는다 — 전에는 거절 앞의 설정이 남았지만 프로세스가 끝나므로 관찰되지 않는다), `configure_run`은 거절 순서와 상태 폴더를 만드는 시점을 그대로 지킨다. `server.py`에는 `main()`·`start()`와 그 단계들(`configure_access`·`configure_run`·`prepare`·`report`·`listen`)과 파서 연결(`build_arg_parser`)이 남는다. `server.py` 1,810 → 1,317행, `C.` 184 → 106곳. 옛 코드와 따로 돌린 프로세스(`python -m limn serve`, 같은 고정 경로와 고정 mtime)에서 81개 명령줄이 같음을 확인했다: `--help`·`--version`·argparse 오류, 접근 거절의 모든 경로와 여러 거절이 겹칠 때의 순서(보안 경계), 포트 거절 셋, 실행 설정 거절 전부(`--doc` 형식 오류 16가지 포함), 시작한 서버 11개의 요약·stderr·상태 폴더 파일과 권한(토큰 수, 읽지 못하는 `tokens.json`, `people.json` 조이기, 원격 저장소의 긴 이름에서 온 라벨, 기본 상태 폴더, IPv6·`localhost` 바인드). 마지막(조립 코드만): 조립이 아닌 것을 바뀌는 이유가 같은 주인에게 옮겼다. 저장소가 믿는 레코드 모양 검사(`valid_rec`와 도우미, 스레드 표시 `THREAD_EVENTS`)는 순수 모듈 `limn/pins/record.py`로 — 저장 레코드에 필드를 더할 때 `limn.pins.edit`·`lifecycle`과 함께 바뀐다. 문서 키 규칙(`limn.documents`)과 행위자 모양(`limn.people`)은 순수하지 않은 모듈의 것이라 인자로 받고, `server.valid_rec`이 둘을 묶는 한 줄이 되어 전처럼 저장소에 넘어간다. 에이전트 응답의 로그 다이어트 `diet_log`(`LOG_TAIL_LINES`)는 응답 직전의 HTTP 일이라 `limn/web/answers.py`로, 처리기가 직접 부른다. `pins.md`의 다시 열림 판단 `pin_reopened_in_round`는 닫기·다시 열기가 스레드에 남기는 표시를 읽으므로 `limn/pins/lifecycle.py`로 옮겼고, 아무도 쓰지 않던 `thread_replies`는 지웠다. 서비스 워커 `SW_JS`는 뷰어 패키지의 데이터 파일 `viewer/sw.js`가 되어 페이지 조각처럼 시작할 때 한 번 읽는다(`assemble.service_worker`). 목록에 없던 것도 옮겼다: 레코드를 API에 보이는 모양(`public`의 규칙 → `limn/pins/view.py`의 `public_record`, 파일 위치를 찾는 일만 `server.public`에 남음), 알림·감사가 적는 행위자 `who`(→ `limn/service/context.py`), 패키지 버전 `app_version`(→ `limn/startup.py`), 처리기만 쓰던 상수 `CLEAR_CONFIRM`·`PAGE_FILE_RE`·`VENDOR_MIME`(→ `limn/web/handler.py`, `App`에서 뺐다). 테스트만을 위해 다시 내보내던 이름(`NOTE_MAX`·`tex_lines`·`anchor_of`·`pin_mentions_all`·`resolve_mentions`와 쓰이지 않던 `atomic_write`·`file_in_tree`·`store_lock`·`truncate_quote`)을 지우고, 다른 모듈이 주인인 이름을 `ps.X`로 읽던 테스트는 주인 모듈에서 가져온다(서버 흐름을 바꾸려고 서버의 이름을 고치는 `patch.object(ps, …)`와 표준 모듈 `ps.time`·`ps.sys`는 그대로). `_ModuleApp`과 한 줄 연결을 명시적인 `App` 객체로 바꾸는 일은 이번에 하지 않았다(결정은 뒤로 미룸). `server.py` 1,339 → 1,151행. 이제 `server.py`에는 실행 설정 `C`와 문서 목록 `DOCS`, 프로세스에 하나인 자원(잠금·캐시·작업 목록·감시 상태), 옮긴 모듈에 이 인스턴스의 설정과 협력자를 묶는 연결, 시작 단계만 있다. 라벨·색을 채우는 `build_html`·`favicon_href`는 `configure_run`이 부르는 시작 단계의 일이라 남겼다. 옛 코드와 따로 돌린 프로세스에서 79개 관찰이 같음을 확인했다(레코드 31가지의 검사 결과와 손상 줄이 섞인 `pins.jsonl`의 읽기·`GET /api/pins`·`GET /pins.md`·쓰기 뒤 손상 원본 보존, 다시 연 스레드 다섯 흐름의 응답과 쓰기마다의 `pins.md`, 실패·`ok_errors`·성공·진행 중 빌드의 `GET /api/build`와 `POST /api/rebuild`(`?log=1` 포함, 예외·바쁨), `GET /sw.js`의 상태·헤더·바이트, `/api/version`, `/pages/`, `/vendor/pdfjs/`, clear 문구 거절, 버림·영구 삭제·clear 뒤 상태 폴더 전체 바이트). 만든 휠을 푼 트리로 돌려도 같았다. 경로 표는 만들지 않는다(분기 순서가 응답을 정한다, 위 앞부분). |
| 7 타입 검사 확대 | 완료 | 2026-09-26 옮긴 모듈(`limn/pins/`, `mapping.py`)부터 mypy strict를 CI 게이트로 켰다 (R8). 모듈을 옮기는 대로 `[tool.mypy]`의 `files`에 더한다. 같은 날 `build.py`·`files.py`를 더했다(표기만 고침, 동작 그대로). 이어서 `store.py`를 더했다. 0.3.4의 새 순수 모듈 `mark.py`도 처음부터 넣었다. 이어서 `web/`을 더했다(처리기가 부르는 서비스는 `App` 프로토콜로 적고, `cast` 한 곳에 이유를 단다). 이어서 `limn` 명령(`cli.py`·`migrate.py`·`__init__.py`·`__main__.py`)을 더하고 두 파일의 docstring을 모두 달았다(R7, 동작 그대로). 이어서 새 모듈 `meta.py`·`outline.py`를 처음부터 넣었다. 이어서 `people.py`·`mentions.py`·`events.py`·`audit.py`를 처음부터 넣었다. 이어서 옮긴 `locate.py`를 더했다(`limn/pins/position.py`는 `src/limn/pins`로 이미 든다). 이어서 `access.py`를 옮기면서 처음부터 넣었다(`guidance.py`는 먼저 들어갔다). 이어서 핀 서비스 `limn/service/`를 옮기면서 처음부터 넣었다(상태를 지키는 `evolve_reply`·`evolve_edit`에는 상태마다 제 타입으로 넘기는 `match`를 둔다). 이어서 `gitsync.py`·`pull.py`를 처음부터 넣었다(`pull.py`의 결과 `match`에서 경우 하나를 빼면 `exhaustive-match`로 실패함을 확인). 이어서 뷰어 조립 `limn/viewer/`(`assemble.py`)를 옮기면서 처음부터 넣었다. 이어서 시작 규칙(`startup.py`·`args.py`·`config.py`)을 처음부터 넣었다. 마지막으로 `server.py`를 넣어(오류 137건, 표기만 고침) 패키지의 파이썬 파일 48개가 모두 strict 검사 대상이다. `server.py` 모듈이 처리기의 `App` 프로토콜을 만족하는지도 mypy가 본다(R8). 이어서 목록을 `files = ["src/limn"]` 하나로 바꿔, 새 모듈은 만들면 따로 등록하지 않아도 검사된다 |

이 문서의 수치(줄 수, 함수 수, docstring 수, Ruff 건수)는 따로 날짜를 적지 않았으면 2026-09-25 Limn 0.2.2 기준 실측이다. 날짜를 적은 수치(§진행 상황, 각 절의 "지금 모습")는 그날 다시 잰 값이다. 단계를 끝낼 때 새로 재서 고친다.
