...
| [`tests/test_pins_lifecycle.py`](../../tests/test_pins_lifecycle.py) | `limn.pins`가 순수한지(import 검사), 레코드가 `pin_state()`와 같은 규칙으로 상태 타입이 되는지, 확인·닫기·다시 열기 전이가 상태마다 맞는 결과를 돌려주고 필드 순서·모르는 필드·입력 레코드를 지키는지, 스레드 id 규칙 |
