...
| [`tests/test_v04.py`](../../tests/test_v04.py) | 0.4(이슈 #7, [ADR-0006](../adr/0006-relative-pin-paths.md)): 핀 파일 위치의 순수 판단(`pin_rel_path`: 원고 안의 `file` 우선, 꼬리가 맞는 `rel_path`, 가장 긴 꼬리, `..`·절대·빈 값 거부), 두 서버 실행 사이에 원고 폴더 이름을 바꾼 흐름(응답의 지금 `file`·`rel_path`, `pins.md` 하위 폴더와 인용, anchor 줄 맞춤, `lo`/`hi` 수정, 겹침, 휴지통·되살리기, 읽기만으로는 다시 쓰지 않음), 원고 밖으로 남아야 하는 경우(꼬리가 맞지 않음, `..`·절대 `rel_path`, 바깥을 가리키는 심볼릭 링크 — 바깥 줄이 새지 않는지), v0.3.0 모듈이 이 상태를 읽고 0.3.0의 위치 다시 잡기가 낡은 `rel_path` 를 이기는지(얕은 클론이면 건너뜀) |
