...
- **도구 줄**(compact): `[핀 N] [선택] [PDF 재빌드] [더보기]` 을 빈칸 없이 같은 높이 한 줄로 잇는다. 폭이 모자라면 글자가 먼저 줄어든다(가로 넘침 없음). 데스크톱 도구 줄은 패널을 좁히면 두 줄로 접힌다.
- **도구 줄 높이**: 버튼·쪽 칸이 한 높이(`--tb-h`: 데스크톱 28px, 터치 44px)이고 아이콘 버튼은 정사각형이다. 쪽 칸은 일반 입력 칸 규칙(글자 14px·여백 6px)을 그대로 받아 버튼보다 8px 크고 글자도 커서 줄이 어긋났다(저자 지적 2026-09-23). 지금은 40×28px·12.5px(터치 52×44px·16px — iOS 확대 방지). 기본 430px 패널에서 데스크톱 도구 줄이 한 줄에 든다(예전에는 [도움말]이 둘째 줄로 떨어졌다).
...
| --- | --- | --- |
| 열림 | 없음 | 카드 |
| 처리 중(claim) | 호박색 `--claim`(다크 `#f0b43c`, 라이트 `#b86e00`) | 카드. 띠는 카드 안쪽 왼쪽에 겹쳐 그린다 — 테두리 폭을 바꾸면 글자가 밀린다 |
| 닫힘 | 초록 `--ok` | 보관함 행(§보관함) |
| 삭제 | 회색 `--arc-grey`(다크 `#7d8796`, 라이트 `#7b8494`) | 보관함 행, 글자가 더 흐리다(`--dim2`) |

띠 색은 두 테마 모두 카드·패널 바탕 대비 3:1(비텍스트) 이상이고(3.7–9.3), 흐린 글자는 4.5:1 이상이다(`--dim2` 4.6 / 4.6).
