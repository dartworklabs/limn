...
- 도구 줄에 남기는 것은 `[핀 N]`(패널·시트 토글, 열린 핀 수)·`[선택]`(터치에서만)·`[PDF 재빌드]`·`[더보기]` 넷이다. 핀 다시 읽기·쪽 이동·축소·확대·폭 맞춤·테마·도움말·닫힌 핀·삭제한 핀은 `[더보기]` 대화상자로 옮긴다(파일·쪽 수·커밋·빌드 시각·작성자 줄도 거기). 원고 수정됨·빌드 진행·연결 끊김·빌드 오류 칩은 접힌 상태에서도 도구 줄 위에 남는다.
...
- **한 간격 체계**: 8px 격자, 같은 줄의 버튼은 같은 높이(터치 44px, 동작 줄 48px). 강조 색(`--primary`, `.btn-default`)은 주요 동작 [핀 저장] 하나에만 칠하고, 카드의 [완료]는 채운 보조(`.btn-secondary`), [삭제]는 옅은 위험 색(`.btn-destructive`)이다. 나머지는 테두리만 있는 outline 이다(§디자인 토큰과 컴포넌트).
- **도구 줄**(compact): `[핀 N] [선택] [PDF 재빌드] [더보기]` 을 빈칸 없이 같은 높이 한 줄로 잇는다. 틈은 4px(`--space-1`)이고, 폭이 모자라면 이름표 칩이 먼저 줄어든다(`flex-shrink` 50, 최소 28px, 전체 이름은 설명에) — 예전에는 버튼 글자가 잘려 'DF 재빌드'처럼 보였다. 데스크톱 도구 줄은 패널을 좁히면 두 줄로 접힌다.
- **도구 줄 높이**: 버튼·쪽 칸이 한 높이(`--tb-h`: 데스크톱 28px, 터치 44px)이고 아이콘 버튼은 정사각형이다. 쪽 칸은 일반 입력 칸 규칙(글자 14px·여백 6px)을 그대로 받아 버튼보다 8px 크고 글자도 커서 줄이 어긋났다(저자 지적 2026-09-23). 지금은 40×28px·`--text-base`(터치 52×44px·`--text-xl` 16px — iOS 확대 방지). 기본 430px 패널에서 데스크톱 도구 줄이 한 줄에 든다 — 이름표가 'Long-DemoPaper1' 처럼 길면 [폭]·테마·도움말이 둘째 줄로 떨어졌다(2026-09-23 실측, 1400×850). 그래서 [핀 다시 읽기]를 열린 핀 목록 머리의 [다시 읽기]로 옮겼고(같은 동작·설명, compact 에서는 [더보기] 안), 이름표 칩은 말줄임으로 줄어들며 전체 이름을 설명(`data-tip`)에 둔다. 지금 도구 줄은 `[이름표] [PDF 재빌드] ··· [쪽] [−] [＋] [폭] [테마] [?]`.
...
- **범위 사다리**: 한 줄 분절 컨트롤. 칸 이름은 짧게(`문단 · 1줄`, `abstract · 19줄`, `frontmatter · 73줄`), 같은 환경 이름이 겹칠 때만 '(바깥)'을 남긴다. 줄 범위는 칸 설명과 aria-label 로 옮겼다. 넘치면 가로 스크롤하고 고른 칸이 보이게 맞춘다. 미세 조정은 붙은 스테퍼 하나(`위 [+][−] 아래 [+][−]`), 그 오른쪽에 [줄바꿈] 토글.
...
카드와 행의 **왼쪽 세로 띠 색**이 상태다. 배지 문구는 뜻이 드러나는 말로 쓴다(저자 지적 2026-09-23: `⏳ 처리 중 · ~04:02`·`#20 안`·`일치 100%` 는 뜻을 알 수 없었다).

| 상태 | 띠 | 모양 |
| --- | --- | --- |
| 열림 | 없음 | 카드 |
| 처리 중(claim) | 호박색 `--status-claimed`(다크 `#f0b43c`, 라이트 `#b86e00`) | 카드. 띠는 카드 안쪽 왼쪽에 겹쳐 그린다 — 테두리 폭을 바꾸면 글자가 밀린다 |
| 닫힘 | 초록 `--status-closed`(= `--success`) | 보관함 행(§보관함) |
| 삭제 | 회색 `--status-dropped`(두 테마 zinc-500 `#71717a`) | 보관함 행, 글자가 더 흐리다(`--subtle-foreground`) |

띠 색은 두 테마 모두 카드·패널 바탕 대비 3:1(비텍스트) 이상이고(3.7–10.0), 흐린 글자는 4.5:1 이상이다(`--subtle-foreground` 다크 5.3 / 라이트 4.8).
...
- **보관함 행**은 카드가 아니다 — 테두리·바탕 없이 왼쪽 띠만 있고 글자가 흐리다.
...
| 상태 | `--success`·`--warning`(+`-foreground`) | 알림 띠·경고 글자 |
| | `--status-open`·`--status-claimed`·`--status-closed`·`--status-dropped`·`--status-warning` | 마크·번호 / 처리 중 띠 / 닫힘 띠 / 삭제 띠 / 위치 잃음·늦어짐(§상태 표현) |
...
| 버튼 secondary | `.btn-secondary` | 카드의 [완료] |
| 버튼 ghost | `.btn-ghost` | [원문 펼치기], 카드 접기, 알림·안내 닫기. 문서 탭(`.dtab`)은 같은 ghost 모양을 자기 규칙으로 그린다(선택된 탭은 PDF 바탕 `--background` 로 이어 붙이고 위에 `--brand` 띠) |
| 버튼 destructive | `.btn-destructive` | 카드의 [삭제] — 옅은 위험 색 바탕 + 위험 색 글자 |
| 크기 | 기본(28px) · `.btn-sm` · `.btn-icon`(정사각형, `.btn-sm` 과 함께면 24px) | 카드·보관함·대화상자 버튼은 sm, 아이콘만 있는 버튼은 icon. 터치에서는 모두 44px 이상 |
| 배지 | `.badge`(= outline) · `.badge-default` · `.badge-secondary` · `.badge-destructive` · `.badge-claimed`(+`.late`) · `.badge-warning` | 카드 배지, 도구 줄 상태 칩(원고가 더 새롭습니다·PNG 보기·연결 끊김·빌드 오류), 셈(`.dcnt`·`.arc-n` = secondary 알약), 문서 칩(`.dchip`), 참조(`.arc-ref`)·보기 전용 표시(`.dvo`) |
| 카드 | `.card` | 핀 카드(`.pin.card`) |
| 입력 | `input`·`textarea` | 메모·쪽 칸. 도구 줄 쪽 칸은 버튼 높이(`--tb-h`) |

예전 표시 전용 클래스(`.p`·`.x`·`.ghost`·`.ib`·`.ico`·`.tag`·`.t`, `.tag.claim`)는 없앴다. `b-close`·`b-drop`·`arc-b` 같은 이름은 역할 표시로 남는다. 모양은 변형 클래스가 정한다. 버튼처럼 눌리지만 모양이 다른 것 — 분절 컨트롤(`.seg`)·스테퍼(`.step`)·구획 머리(`.arc-head`)·[원래 요청](`.arc-orig-t`, 밑줄 글자) — 은 같은 토큰을 쓰는 별도 모양이다.
...
| 화면 | 전환 수단 |
| --- | --- |
| 데스크톱·펼친 폴드(`wide`·`mid`) | PDF 영역 위 탭 줄(`role=tablist`). 탭 = 이름 + 열린 핀 수 배지 + 보기 전용 `PDF` 표시 + 원고 수정됨 점·빌드 중 표시. 지금 탭은 PDF 영역과 같은 바탕으로 이어 붙이고 위에 이름표 색 띠. 넘치면 가로 스크롤 |
| 접은 폴드(`narrow`) | 도구 줄의 `[문서 이름 ▾]` → 아래 시트 목록(이름·경로·핀 수·보기 전용). 탭 줄은 세로 공간을 먹고 좁은 폭에서 서너 개밖에 안 보여 쓰지 않는다 |
| 문서 하나 | 탭 줄·문서 버튼·'모든 문서' 토글이 모두 숨는다 — 지금 화면 그대로 |
...
- **키보드**: Ctrl+PgUp/PgDn(이전·다음), Alt+1…9(e.code — 맥의 Option+숫자는 다른 글자를 낸다), 탭에 포커스가 있으면 ←/→/Home/End. 입력 칸에서는 동작하지 않는다. 브라우저가 이 키를 먼저 가져가면(크롬의 탭 전환) 탭을 누른다.
...
- **빌드**: [PDF 재빌드]는 지금 문서만, 보기 전용이면 숨긴다. 다른 문서가 낡거나(점)·빌드 중(스피너)이면 탭에 표시하고, 뒤에서 빌드가 끝나면 알림에 [열기]를 붙인다(`build_seq` 를 문서별로 센다).
...
| 버튼 [＋]·[−]·[폭] | 같은 1.2배 단계와 폭 맞춤(PDF 영역 가운데 기준) |
