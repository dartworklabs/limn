...
| `mid` | 701–1099px, 입력 장치와 무관 | 기본 330px. 701–900px은 처음에 접힌 오버레이, 901–1099px은 처음에 열린 문서 옆 패널. 접으면 오른쪽 아래 도구 줄 | 처음에는 접힘. 펼치면 문서 위 오버레이; 핀 패널과 하나씩 연다 |
...
- **상단 공간**: 문서 링크는 한 줄을 유지하며 필요하면 그 줄 안에서 가로로 스크롤된다. 중간 폭은 이름표를 [더보기]에 두고, 보기 전환과 목차 버튼은 줄어들지 않는다. 탐색 줄은 마우스 44px·터치 48px이다. 첫 안내는 탐색 줄 아래에 뜨며 안내 글은 클릭을 가로채지 않고 닫기 버튼만 입력을 받는다.
