...
| 무엇 | 모양 |
| --- | --- |
| 이동하는 글자(`#번호`·`N쪽`·글 속 `#12`·보관함 [원래 요청]·[변경 보기]) | 점선 밑줄, hover·초점에 실선. 정적 글자(보관함 행의 `#번호`, 배지 속 `#20과 같은 범위`, 작성자 이름)는 밑줄이 없다 |
| 복사하는 글자(줄 범위 `.loc`) | 주 색 고정폭 글자 + `cursor:copy`, 설명 '누르면 복사' |
...
| 긴 글 | 스레드 글은 6줄에서 접고 [더 보기](1,000자 답글 하나가 카드를 1,390px 로 늘렸다 → 약 310px) |
...
- **`#번호` 링크**: 글 속 `#12` 는 그 핀이 있으면(열림·검토 대기·완료·삭제) 점선 밑줄 링크(`.pin-ref`)다 — 누르면 그 카드·보관함 행을 펴고 스크롤해 반짝인다(다른 문서면 '모든 문서'를 켠다). 없는 번호와 `&#39;` 같은 이스케이프는 건드리지 않는다.
- 사람을 부른 카드에는 `나를 부름`·`@이름` 배지.
