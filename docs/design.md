...
| `mid` | 700px 초과 1100px 미만 + 터치(`pointer:coarse`) | 좁은 패널(`clamp(300px,38vw,360px)`), 접으면 오른쪽 아래 도구 줄만 | 880×790: 0.436 → 0.575(접으면 0.95) |
| `narrow` | 700px 이하 | 하단 시트, 기본은 접힘(아래 도구 줄만) | 412×915: 0.46(가로 넘침) → 0.903 |
...
- 모바일 레이아웃은 헤드리스 크롬 에뮬레이션(Playwright, `isMobile`·`hasTouch`)으로만 쟀다. 실기기의 가상 키보드는 visualViewport 높이를 줄이는 흉내로, 핀치 확대는 두 손가락 CDP 터치로 대신했다. 실제 갤럭시 Z 폴드 7 에서 한 번 더 봐야 한다.
