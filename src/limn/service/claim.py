...
def claim_pin(ctx: PinContext, pid: int, actor: Mapping[str, Any], ttl_min: int,
    ...
    def fn(rows: list[Row]) -> tuple[OpenPin | ClaimClosedPin | ClaimedByOther | PinNotFound, bool]:
        ...
        result = claim(parse_pin(r), typed_actor(actor), ctx.epoch(), ctx.now(), ClaimRequest(ttl_min, eta_min),
...
def unclaim_pin(ctx: PinContext, pid: int,
                actor: Mapping[str, Any]) -> OpenPin | ReviewPin | DonePin | NotClaimed | PinNotFound:
