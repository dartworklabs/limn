...
def rel_badge(rel: Sequence[Mapping[str, Any]], by_id: Mapping[int, Record], me: Record | None = None) -> str:
    ...
    if me is not None:
        same = [
            x
            for x in rel
            if (by_id.get(x["id"]) or {}).get("lo") == me.get("lo")
...
LEGEND = (
    "표시: '#N 범위 안'·'#N과 같은 범위' = N과 한 번에 고치고 둘 다 닫는다 · '#N과 일부 겹침' = 참고만, 각자 처리해도 된다 · "
...
def token_guidance_line(shown_file: str | None) -> str:
    ...
    return (
        TOKEN_GUIDANCE
        + " · 이 기기의 에이전트는 토큰 파일을 붙인다: "
        + token_file_curl(shown_file)
        + "(파일 내용은 출력하지도 저장소에 옮기지도 않는다)"
    )


REPLY_GUIDANCE = (
    "답글(0.2.2): 사람 신원으로 단 답글은 검토 대기·완료 핀을 다시 연다(답글이 다시 연 이유가 된다) — "
...
def review_md(rows: Sequence[Record], facts: Mapping[int, PinFacts], sectioned: bool) -> list[str]:
    ...
    out = [
        "",
        "## 검토 대기 %d건 — 사람이 확인할 차례. 에이전트는 다시 처리하지 않는다(다시 열리면 위 열린 표로 돌아온다)"
        % len(rows),
        "",
        "| # | 위치 | 확인할 사람 | 닫을 때 남긴 답 |",
        "|---|---|---|---|",
...
def pins_md_text(page: PinsMdInput) -> str:
    ...
    if not sectioned:
        ...
        out.append(
            "핀은 아래 문서별 소절(`## 이름 · 키 · 경로`)로 묶였다 — 위치 칸의 경로는 `--manuscript` 기준. "
            "소절의 `기준:` 커밋이 다른 체크아웃에서 처리하면 먼저 `git rev-parse --short HEAD` 가 같은지 확인"
        )
    out.append(
        "갱신: %s  ·  열린 핀 %d건  ·  %s닫힌 핀 %d건(뷰어의 '닫힌 핀'에서 확인)"
        % (
            page.updated,
            len(openn),
            "검토 대기 %d건(맨 아래, 처리하지 않는다)  ·  " % len(reviewn) if reviewn else "",
            n_done,
        )
    )
    out.append("")
    guidance = (
        "처리한 핀은 닫는다 — 닫을 때 `changes` 에 이 핀 때문에 바꾼 줄 범위를, `ref` 에 `PR #번호 (커밋 해시)` 를 적는다: "
    ...
    if n_human:
        guidance += (
            " · 번호 칸에 `→ @이름` 이 붙은 핀 %d건은 담당이 사람인 핀이다 — 요청한 사용자가 그 핀을 "
    ...
    if page.repo:
        guidance += (
            " · 처리 전 자기 체크아웃의 `git remote get-url origin` 이 위 저장소와 같은지 확인. "
            "다르면 다른 논문의 핀이니 멈춘다"
        )
    if n_region:
        guidance += (
    ...
    for k, rs in rows_by_doc.items():
        ...
        shown += 1
        out += (
            ["", "## 설정에 없는 문서 · `%s` — 이 뷰어의 --doc 목록에 없다. 처리 전에 사용자에게 확인" % md_cell(k), ""]
            + header
            + rs
        )
    if not shown:
