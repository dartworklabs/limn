...
def serve_parser(description: str, version: str, default_envs: str) -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        "(%d characters or fewer). Defaults to --manuscript's git origin repo name, or the folder name if not a git repo"
        % LABEL_MAX,
    )
    ap.add_argument(
        "--accent",
    ...
    acc.add_argument(
        "--proxy-email-header", help="Optional header carrying the e-mail; when present it is used as the login"
    )
    acc.add_argument(
        "--members-only",
        action="store_true",
    ...
    acc.add_argument("--local-user", help="The owner's login under --auth local (default $USER, then 'owner')")
    acc.add_argument(
        "--agent-token-file",
        default=os.environ.get("LIMN_AGENT_TOKEN_FILE") or None,
        metavar="PATH",
