...
class DecideEdit(unittest.TestCase):
    ...
    def test_stale_base_rev_is_a_conflict_checked_after_the_closed_rule(self):
        ...
        self.assertIsInstance(decide(OpenPin.from_record(line_record(rev=None)), base_rev=0, note="n"), PinEdited)
        closed = DonePin.from_record(line_record(done=True))
    ...
    def test_merged_note_over_the_limit_is_refused_with_its_length(self):
        ...
        pin = OpenPin.from_record(line_record(note="x" * (NOTE_MAX - 5)))
    ...
    def test_range_changes_when_moved_or_when_the_pin_was_stale(self):
        ...
        stale = decide(OpenPin.from_record(line_record(stale=True)), count=10, base_rev=2, lo=4, hi=5)
...
class EvolveEdit(unittest.TestCase):
    ...
    def test_unmoved_lines_keep_matching_fields(self):
        ...
        out = evolve_edit(OpenPin.from_record(line_record()), edited(lines=(4, 5)), None, None, None, None).record
    ...
    def test_note_mentions_and_note_level_fields_then_edit_stamp_and_rev(self):
        ...
        pin = DonePin.from_record(line_record(done=True, mentions=["bob@example.com"]))
