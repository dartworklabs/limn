...
class Sandbox:
    ...
    def __init__(self, root: Path):
        ...
        self.env = dict(
            ...
            LIMN_CONFIG_DIR=str(self.cfg),
            LIMN_DATA_ROOT=str(self.data),
        )
...
class ServerTokenFile(AccessBase):
    ...
    def test_agent_token_file_flag_defaults_to_the_environment(self):
        ...
        code = (
            "import sys; sys.path.insert(0, %r); from limn import server as s; "
        ...
        def parsed(*args: str) -> str:
            return subprocess.run(
                [sys.executable, "-c", code, *args], capture_output=True, text=True, env=env, timeout=60, check=False
            ).stdout.strip()
...
class InstanceManagerProbes(SandboxTest):
    ...
    def setUp(self):
        ...
        self.box.env.update(
            ...
            XDG_RUNTIME_DIR=str(self.box.root),
            LIMN_USER_UNIT_DIR=str(self.box.root / "units"),
            LIMN_WAIT="20",
    ...
    def write_config(self) -> None:
        ...
        (self.box.cfg / "paper.env").write_text(
            ...
            % (self.box.ms, self.port, self.port - 100, self.box.state),
            encoding="utf-8",
        )
    ...
    def test_a_token_file_open_to_others_is_not_used(self):
        ...
        self.assertNotIn(
            self.box.token_file.read_text(encoding="utf-8").strip(), self.curl_log.read_text(encoding="utf-8")
        )
