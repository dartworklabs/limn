...
# the comparison worker's own "build_failed" status; limn/scope.py, limn/documents.py: the refusal values), the access
# boundary (limn/access.py: identify, admit, check_role and bearer_of raise their refusals) and the HTTP layer (limn/web: the handler, the parsers, the answers, the refusal tables). Every static guard below reads all
...
SOURCES = {p.name if p.parent.name != "web" else "web/" + p.name: p.read_text(encoding="utf-8")
           for p in [Path(ps.__file__), PKG / "revisions.py", PKG / "scope.py", PKG / "documents.py", PKG / "access.py"]
