...
line re-sync, no range edit, no «…» quote, and a bare file name in pins.md. Records now also carry `rel_path` (relative
to --manuscript) when the server writes them, and every record - old ones included - is located on read by one rule
(pin_rel_path): the stored file if it lies under the current root, else a rel_path that matches the file's tail, else
the longest tail of the file that exists under the root. Old records are never rewritten just to add rel_path, and
nothing outside the tree is ever read.
...
class PinRelPathRule(unittest.TestCase):
    ...
    def pick(self, file, rel_path=None, under=None, existing=()):
        return ps.pin_rel_path(file, rel_path, under, set(existing).__contains__)
    ...
    def test_a_rel_path_matching_the_file_tail_locates_a_moved_record(self):
        """rel_path is trusted without an existence check when the stored file ends with it (the server writes both)."""
    ...
    def test_a_rel_path_that_no_longer_matches_the_file_is_ignored(self):
        """A 0.3.0 relocation changes file but leaves rel_path: the stale rel_path must not win (ADR-0006 §2.2)."""
    ...
    def test_unsafe_or_malformed_rel_paths_are_ignored(self):
        """Absolute, parent-escaping, empty or non-string rel_path values are never used."""
    ...
    def test_legacy_records_take_the_longest_existing_tail(self):
        """Without rel_path, the longest tail of the stored path that exists under the root wins (to_source()'s rule)."""
...
class MovedManuscriptBase(AccessBase):
    ...
    def setUp(self):
        ...
        self.p_quote = ps.add_pin({"file": str(self.a / "sections" / "long.tex"), "lo": 1, "hi": 1, "scope": "raw",
                                   "quote": "Long line word word", "note": "q"}, dict(ps.LOCAL_ACTOR))
        self.make_legacy(self.p_old, self.p_quote)            # as 0.3.0 wrote them: no rel_path
    ...
    def pin_at(self, rel, lo, hi, note="n"):
        return ps.add_pin({"file": str(ps.C.src / rel), "lo": lo, "hi": hi, "note": note}, dict(ps.LOCAL_ACTOR))
    ...
    def make_legacy(self, *ids):
        self.rewrite(lambda rows: [r.pop("rel_path", None) for r in rows if r["id"] in ids])
...
class MovedManuscript(MovedManuscriptBase):
    ...
    def test_new_records_store_a_relative_path_next_to_the_absolute_one(self):
        """A pin created by this version stores rel_path and a file under the current root."""
        r = self.stored()[self.p_new]
        self.assertEqual((r["file"], r["rel_path"]), (str(self.a / "sections" / "x.tex"), "sections/x.tex"))
    ...
    def test_range_edit_works_on_a_legacy_record_and_stamps_the_new_location(self):
        """/edit with lo/hi used to be 400 'outside the manuscript'; now it edits, and the record gets rel_path + the current file."""
        self.move()
        ...
        self.assertEqual((r["file"], r["rel_path"]), (str(self.b / "sections" / "x.tex"), "sections/x.tex"))
        self.assertEqual(r["anchor"]["head"], "Sentence number 14 about topic14.")

    def test_reading_never_rewrites_a_legacy_record(self):
        """No write migration: reads after the move leave pins.jsonl byte for byte as it was."""
        self.move()
        ...
        self.assertNotIn("rel_path", self.stored()[self.p_old])
    ...
    def test_a_write_for_another_pin_does_not_backfill_a_legacy_record(self):
        """Only writes to the pin itself (create, edit, restore) add rel_path; other writes keep the stored record."""
        self.move()
        self.pin_at("sections/x.tex", 18, 18)
        code, _ = self.call("POST", "/api/pins/%d/close" % self.p_new, {"reply": "done"})
        self.assertEqual(code, 200)
        r = self.stored()[self.p_old]
        self.assertNotIn("rel_path", r)
    ...
    def test_drop_and_restore_after_the_move(self):
        """The Trash shows the current path, and a restored legacy record comes back with rel_path."""
        self.move()
...
class OutsideTheTree(MovedManuscriptBase):
    ...
    def put(self, **fields):
        ...
        def fn(rows):
            ...
            for k in ("anchor", "rel_path", "synced_at", "scope"):
                r.pop(k, None)
    ...
    def test_a_parent_escaping_rel_path_is_not_followed(self):
        """rel_path '../outside/secret.tex' (with a file ending the same way) does not climb out of the root."""
        f = str(self.b / ".." / "outside" / "secret.tex")
        self.put(file=f, rel_path="../outside/secret.tex")
        self.assert_out_of_tree(f)

    def test_an_absolute_rel_path_is_not_followed(self):
        """An absolute rel_path would replace the root when joined; it is ignored."""
        f = str(self.outside / "secret.tex")
        self.put(file=f, rel_path=f)
        self.assert_out_of_tree(f)

    def test_a_symlink_inside_the_tree_pointing_outside_is_refused(self):
        """Neither a trusted rel_path nor a tail may reach an outside file through a symlink in the manuscript."""
        (self.b / "linked").symlink_to(self.outside, target_is_directory=True)
        f = "/nowhere/paper-a/linked/secret.tex"
        self.put(file=f, rel_path="linked/secret.tex")
        self.assert_out_of_tree(f)
        self.put(file=f)                                            # legacy: the tail linked/secret.tex exists via the link
        self.assert_out_of_tree(f)

    def test_a_rel_path_of_the_wrong_type_is_a_broken_line(self):
        """rel_path, when present, must be a string - like every field the store reads."""
        self.rewrite(lambda rows: [r.update(rel_path=5) for r in rows if r["id"] == self.p_old])
...
class RollbackToV030(MovedManuscriptBase):
    ...
    def test_v030_reads_the_branch_state_unchanged(self):
        """Same pins, lines and files from 0.3.0 (rel_path is just an extra field), and the same pins.md."""
        new = self.api_pins()
    ...
    def test_a_v030_relocation_wins_over_the_stale_rel_path(self):
        """0.3.0 relocating a pin to another file changes file but keeps rel_path; the branch follows the new file."""
        ...
        self.assertEqual(code, 200, d)
        self.assertEqual(self.stored()[self.p_new]["rel_path"], "sections/x.tex")   # 0.3.0 left it as it was
