        ...
        for name in ("startup.py", "args.py", "config.py"):
            ...
            with self.subTest(name):
                ...
                exits = [
                    ...
                    if isinstance(c, ast.Call) and isinstance(c.func, ast.Attribute) and c.func.attr == "exit"
                ]
...
class AccessRefusals(unittest.TestCase):
    ...
    def test_tailnet_agent_needs_the_loopback_agent(self):
        ...
        for argv, auth, bind, extra in (
            (("--no-agent-loopback",), "tailscale", "127.0.0.1", ", --no-agent-loopback"),
            (("--auth", "local"), "local", "127.0.0.1", ""),
            (("--auth", "trusted-proxy"), "trusted-proxy", "127.0.0.1", ""),
            (("--bind", "0.0.0.0", "--i-know-this-is-insecure"), "tailscale", "0.0.0.0", ""),
        ):
    ...
    def test_the_first_fault_in_rule_order_is_the_one_reported(self):
        ...
        self.assertTrue(
            refusal("--auth", "local", "--agent-loopback", "--tailnet-agent").startswith("--agent-loopback")
        )
        self.assertTrue(refusal("--public-host", "x/", "--trusted-proxies", "bad").startswith("--public-host"))
        self.assertTrue(
            refusal("--proxy-user-header", "a b", "--local-user", "agent:x").startswith("--proxy-user-header")
        )
...
class AccessAccepted(unittest.TestCase):
    ...
    def test_values_are_parsed(self):
        ...
        got = options(
            ...
            "trusted-proxy",
            "--public-host",
            "A.example.com:8443,b.example.com",
            "--trusted-proxies",
            "10.0.0.0/8",
            "--proxy-email-header",
            "X-Email",
...
class AccessLog(unittest.TestCase):
    ...
    def test_default_line_and_deprecation_warning(self):
        ...
        self.assertEqual(
            startup.access_log_lines(options(), 0, False),
            [
    ...
    def test_trusted_proxy_on_a_public_address(self):
        ...
        self.assertEqual(
            startup.access_log_lines(got, 2, True),
            [
    ...
    def test_insecure_and_tailnet_warnings(self):
        ...
        lines = startup.access_log_lines(
            options("--auth", "local", "--local-user", "bob", "--bind", "0.0.0.0", "--i-know-this-is-insecure"),
            0,
            False,
        )
        self.assertEqual(lines[0], "auth        local · owner bob · tokens 0 · loopback agent off · members-only off")
        self.assertTrue(
            lines[2].startswith("warning     !!! --i-know-this-is-insecure: --auth local on 0.0.0.0 is NOT")
        )
...
class Port(unittest.TestCase):
    ...
    def test_listen_refusal_names_a_taken_port_or_the_os_reason(self):
        ...
        self.assertEqual(
            startup.listen_refusal("127.0.0.1", 18999, OSError(errno.EADDRINUSE, "in use")),
            StartupRefused(
                "limn serve: port 18999 on 127.0.0.1 is already in use - stop the other server, or pick another --port"
            ),
        )
        self.assertEqual(
            startup.listen_refusal("10.0.0.9", 80, OSError(errno.EADDRNOTAVAIL, "Cannot assign")),
    ...
    def test_probe_refuses_a_taken_port_and_passes_a_free_one(self):
        ...
        with socket.socket() as s:
            ...
            self.assertEqual(
                startup.probe_port("127.0.0.1", port), StartupRefused(startup.port_in_use_message("127.0.0.1", port))
            )
...
class Documents(unittest.TestCase):
    ...
    def test_state_dir(self):
        ...
        self.assertEqual(
            startup.state_dir(None, self.ms, Path("/d")), Path("/d") / "limn" / "serve" / startup.state_slug(self.ms)
        )
...
class Parser(unittest.TestCase):
    ...
    def test_given_values_reach_help_version_and_defaults(self):
        ...
        with mock.patch.dict(os.environ, {"LIMN_AGENT_TOKEN_FILE": "/x/t.token"}):
            self.assertEqual(
                args.serve_parser("d", "v", "e").parse_args(["--manuscript", "m"]).agent_token_file, "/x/t.token"
            )
...
class RepoNameFromUrl(unittest.TestCase):
    ...
    def test_ssh_url_with_path(self):
        self.assertEqual(startup.repo_name_from_url("git@github.com:example-lab/paper-a.git"), "paper-a")
...
class DefaultLabel(unittest.TestCase):
    ...
    def test_uses_repo_name_when_remote_given(self):
        ...
        src.mkdir()
        self.assertEqual(startup.default_label(src, "git@github.com:example-lab/paper-a.git"), "paper-a")
    ...
    def test_git_remote_url_reads_origin(self):
        ...
        subprocess.run(["git", "remote", "add", "origin", "git@example.com:org/paper-x.git"], cwd=src, check=True)
