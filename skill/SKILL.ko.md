...
- `reply`·`ref`(선택으로 `changes`)를 남겨 닫는다(6단계). 토큰 없이 테일넷 주소로 닫으면 `"review": true` 를 넣는다.
- 고친 뒤 PDF를 재빌드한다(§규칙).
...
5. **고친다 — 핀 하나에 커밋 하나.** `L<lo>-L<hi>` 를 `Read` 로 열어 문맥을 확인하고 메모대로 고친다. 앞 핀을 고쳐 줄이 밀렸을 수 있으면 `GET <base>/api/pins` 로 맞춘 값을 다시 받는다.
   - **핀마다 따로 커밋한다**(v0.3부터): 핀 하나의 수정 → 커밋 하나. PR 하나에 그런 커밋이 여럿이어도 된다. 그러면 커밋이 곧 핀이라, 검토하는 사람의 [변경 보기]가 소스 diff와 비교 PDF 모두에서 그 핀의 변경만 보인다. 6단계의 `changes` 에 쓸 수 있게 이 핀 때문에 바꾼 줄(커밋 뒤 번호)을 적어 둔다.
   - 여러 핀의 PR은 스쿼시하지 말고 머지 커밋이나 리베이스로 합친다. 스쿼시하면 핀별 커밋이 `main` 에서 사라진다. 어차피 스쿼시한다면 `ref` 에 둘 다 적는다(`PR #12 (abc1234)`). 그러면 뷰어가 스쿼시 커밋을 찾을 수 있다.
6. **닫는다.** 에이전트가 닫은 핀은 완료가 아니라 **검토 대기**로 간다 — 사람이 뷰어에서 [확인]하면 완료다. 사람이 [답글]로 틀린 점을 적으면 그 답글이 다시 연 이유가 되어 열린 표로 돌아온다(`다시 열림`). 사람을 @태그한 답글은 사람끼리의 대화라 상태가 그대로이고, 검토 대기 핀은 계속 처리하지 않는다.
   - **수정 요청 핀**: 고친 뒤 무엇을 고쳤는지(`reply` ≤500자)와 **그 핀의 커밋 해시**(`ref` ≤80자, PR 번호를 함께 적어도 된다: `PR #12 (abc1234)`)를 남겨 닫는다. `ref` 는 뷰어의 [변경 보기]가 그 커밋을 찾는 단서다.
   - 선택으로 `changes` 를 넣는다: `[{"file": "<위치 칸의 경로>", "lo": <첫 줄>, "hi": <끝 줄>}, …]` — **이 핀 때문에** 바꾼 줄 범위이고, 번호는 그 커밋 기준(고친 뒤)이다. 50개까지이며, 원고 폴더 밖 파일·정수가 아닌 줄·`lo > hi` 는 `400` 이고 아무것도 바뀌지 않는다. 한 커밋이 핀 여럿을 고쳤을 때 뷰어가 핀마다 제 hunk만 보이게 하는 것이 이 필드다. 없으면 서버가 핀 범위로 추정한다.
...
| `POST /api/pins/{id}/close` | 닫는다. 본문 `{"reply", "ref", "changes", "review"}` — 에이전트가 닫으면 검토 대기. `changes`(v0.3, 선택) = `[{file, lo, hi}]`, 이 핀 때문에 바꾼 줄 |
